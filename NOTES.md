...
3. **M3 记忆与上下文**：多轮状态、长上下文、RAG 与检索评估（第 7 课 多轮状态+上下文管理 ✅ 2026-09-08；第 8 课 检索注入/RAG 上手 ✅ 2026-09-10；第 9 课 检索评估 eval 首次上手 ✅ **实证收束 2026-09-12**。**M3 三课收官**：管得住 → 取得回 → 量得准。第 9 课按用户选择走"**评估驱动检索调优**"主线（未装向量库依赖）：语料 8→30 块、22 题 gold set、四路检索器对比（关键词 0.73 / 语义 0.91 / 混合 0.91 / 语义加权 0.95，Hit@1）、阈值按正/负样本分布标定、追问改写让命中 2/5→5/5。真向量库（Chroma/FAISS）与持久化刻意留给 M4 之后或直接长进作品集项目 M7。**挑战 A/B 收束实绩**：题集 22→27 题、加"只看标题"的检索器（路 E 踩中"整句子串判断 → 稳定排序退化成文件顺序"）、独立写出 `_calibrate-min-score.py` 标定 `MIN_SCORE`（并自己补出第二类负样本"手册里根本没有的问题"）、量出"阈值随语料扩张失效"的机制（加块不改已有块分数，但噪音天花板 max 只涨不跌，22/27 道题上涨）；挑战 B ③（改写）与检索层升级为关键词+语义 RRF 合流由教师代笔落地预跑，**挖到一个教学价值极高的失败案例**：题 02 的并列名次被文件顺序断（"RTO" 与 "CTO+空格" 打成平手）。**下一课：M4「手写 vs 框架」**——手写循环到此为止，看 LangGraph / Pydantic AI / OpenAI Agents SDK 封装了什么、何时该上何时别上）
...
- 交付物 `reference/m2-m3-review.html`：卷 A 概念自测 **22 题**（**交错出题、不标课号**，反馈里才标课号，让用户自己统计塌方）/ 卷 B 数字默写 **14 题**（`<details>` 折叠，先默写后对答案）/ 错题映射表（错在哪几题 → 读哪篇原典哪一节 → 回看哪个 code 的哪面镜）/ 3 篇原典带读问题（每篇 5 问，全部锚在用户已做过的项目上）/ 九课速查卡 + 面试视角六问 / 错题本与验收标准。设计记录见 `learning-records/0011`。<br>**2026-09-12 更新（第 9 课实证收尾后）**：题数 20→22、12→14，补进挑战 A/B 的实证产出（三类负样本 / 阈值随语料失效的机制 / 合流后闸门挂哪 / "排序失效 ≠ 检索器差"），并更新被 27 题题集刷新过的旧数字。
...
- 使用时机：M3 正式收束（用户完成第 9 课挑战 A/B）之后、M4 开始之前。<br>**✅ 时机已到（2026-09-12 第 9 课实证收束完成）**——下次开工默认先上复盘日，不要直接进 M4。<br>做完后据结果决定 M4 是否要先补 M2 的洞。
