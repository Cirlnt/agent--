...
## 第 9 课 · 给 RAG 装一把尺子：检索评估（eval 第一次上手）· M3（第三课 · 收官）

### ① 一句话核心
**评估（eval）= 一份自己标注的评估集（题 + 每题该命中的块 gold） + 几个客观指标（Hit@1 / Hit@k / Recall@k / MRR）。** 有了它，"感觉变好了"被翻译成"Hit@1 从 0.73 涨到 0.91"，调参才从玄学变成工程。**没有尺子，你只能看到"变化"，看不到"改进"**——这是本课要立的工程习惯，也是 M6 生产化 evals 的前半段（本课只量**检索**那一端，带 faithfulness / answer relevancy 的**生成**那一端留在 M6）。

### ② 心智模型
- **四个刻度（要能默写）**：`Hit@1` top-1 里有没有 gold；`Hit@k` top-k 里有没有 gold；`Recall@k` **该拿的 gold 拿全了几成**；`MRR` 第一个 gold 的名次取倒数（第 1 名 1.0 / 第 3 名 0.33 / 没进 top-k 记 0）。
- **Hit 与 Recall 会分家**：答案横跨 3 块只捞到 1 块 → Hit@3 满分、Recall@3 = 0.33。**只看 Hit 会以为自己挺好，其实模型每次都在半块材料上作答**。
- **MRR 比 Hit 敏感**：它关心"排多靠前"，"答案从第 1 掉到第 3"这种退化只有它看得出来。
- **题集是资产：要反向出题、要评审、要长**。**从语料反向出题**（先想该命中哪块，再写问句）；覆盖三类问法（**术语型 / 口语换说法型 / 追问型**）；**指标异常时先怀疑题，再怀疑检索器**（本课真实踩中）；线上每被问住一次就补一道题 = 回归测试集。
- **混合检索用 RRF，而不是加权分数**：RRF 只吃**名次**、不吃分数 → 名次无量纲 → 上一课那条"量纲不可跨路搬运"被顺手绕过去。**但合流治"漏"不治"排第一"**：RRF 会把某一路的错答案一起顶上来（本课实测两例），所以它的收益在 **Hit@k / Recall@k** 那一栏，**不在 Hit@1**。
- **阈值不是常数，是数据分布的一次快照**：算【正样本】（答案块的最高分）与【负样本】（噪音天花板）两条分布，看重叠区。**分得开** → 空隙里随便挑；**分不开** → 要么提升检索质量（合流 / rerank / 换 embed），要么按"更怕误杀还是更怕漏放"明说选哪一边——**唯独不该继续试数字找"那个对的数"**。且阈值**不能跨路、不能跨语料搬运**。
- **query 自足性**：不自足的查询（"那另外那部分呢"）脱离上文没有主题，向量压根不在目标语义邻域里。**改写必须是显式的一步**（system 里明确要求 query 自足，或专门做改写环节）——**别赌模型自己发生**；而且**改写也会弄丢、会理解偏**（命中 ≠ 改写正确），同样得用同一把尺子验收。
- **尺子自己有分辨率**：22 题上名次差一名 = 4.5%，"0.91 vs 0.95"只差一道题，**不足以**下结论。读指标先问"这个差距比噪音大吗"。**评估集自己也会被调参过拟合** → 留一批题不参与调参、只做验收。
- **RAG 的四颗旋钮**（都在同一把尺子下量，且**一次只动一颗**）：检索器选型 / 融合权重 / 分数阈值 / top_k。

### ③ 要点清单
- [ ] **四指标定义**脱口而出；能说清"Hit 满分而 Recall 0.33"是怎么发生的。
- [ ] **先有尺子再调优**（decisive）：上一课刚把 RAG 做通，第一件事不是打磨提示词、不是换模型、不是上向量库，而是**先写评估集 + 算指标**——否则后面每一次改动都无法判断是改进还是退化。
- [ ] **四路指标实测**（30 块语料 / 22 题）：关键词 `Hit@1 0.73` ｜ 语义 `0.91` ｜ 混合 RRF `0.91`（Hit@3/Recall@3 满分）｜ 混合·语义×2 `0.95`。**互补 > 单路**。
- [ ] **阈值标定三步**：算正样本分布 → 算负样本分布 → 看重叠区并按场景取舍；能解释上一课拍的那个 `0.6` 错在哪（30 块语料上误杀 30%）。
- [ ] **追问改写值多少**：同一套检索器，追问原话 Hit@1 **2/5** → 显式改写后 **5/5**。
- [ ] **top_k 的拐点**：本批数据 Hit@k 到 k=2 就满、Recall@k 到 k=3 也满；再往后 5/8/30 指标纹丝不动，平均注入 token 91 → 197 → 302 → 466，**全量塞 30 块 = 1576 token（k=3 的 8.0 倍）而指标一个点没涨**。
- [ ] **能诚实说出尺子的三个误差来源**：① 题和语料都是自己写的 → 偏乐观，真实语料更难；② 分辨率有限（22 题上差一名 = 4.5%）；③ 同一个评估集反复调参会过拟合。

### ④ 实测发现（DeepSeek v4-flash / Anthropic 兼容端点；教师预跑 2026-09-10 + 用户实证补记见文末）
- **镜 1 · 四路一起上秤（22 题，确定性）**：
  ...
  - **两路各自的主场（决定性的现场证据）**：题 02「RTO 是多少」→ gold C21（数据备份与恢复）。关键词 top-1 = C06（C21 排第 2）；**语义 top-3 = C05/C19/C27，连 C21 都没进**；混合 ✅C21 第 1。→ **精确串（错误码、缩写）是 embedding 的盲区、字面路的主场**，合流把它救回——正是 Anthropic《Contextual Retrieval》那条论点的现场版。
  - **反向案例（诚实）**：题 15「代码要几个人 approve」→ 语义 ✅C19、混合 ❌C20；题 20「供应商货款」同型。→ **合流治漏不治排第一**，故混合 Hit@1 与语义持平、但在 Hit@3/Recall@3 全胜。
  - 22 题中 **8 道出现"各路分歧"**，其余 14 道四路一致——**对选型没有信息量**。评估集里真正推动决策的是分歧题。
- **镜 2 · 阈值标定（决定性，回答上一课悬案）**：余弦路正样本 0.819…0.644…0.424、负样本 0.647…0.561…0.362 → **重叠区 [0.424, 0.647]**。阈值 0.70 → 误杀 17 (77%)；**0.60（上一课那个值）→ 误杀 7 (32%) / 漏放 3 (14%)**；0.50 → 漏放 20 (91%)。关键词路正样本 0.692…0.062、负样本 0.333…0.000 → 重叠区 [0.062, 0.333]，0.15 档误杀 11 (50%)。**两路量级完全不同** → "量纲不可跨路搬运"从道理变成两张分布表。**阈值是数据分布的快照，语料一变就失效。**
- **镜 3 · query 自足性（决定性，回答上一课悬案）**：5 道追问 **A 原话 2/5 ｜ B flash 改写 5/5 ｜ C 人工自足 5/5**（真调模型改写、原样打印）。**诚实发现**：其中一句改写把意思**理解偏了**（原追问指门诊，改写成"住院费用中不能报销的部分"），只因主题词还在就照样捞对 → **命中 ≠ 改写正确，改写质量仍要人看**。
- **镜 4 · top_k 扫描**：见 ③ 的拐点数字；"全量塞"是上一课"为什么不全塞"的量化收尾。
- **教师出题时真踩的坑（已写进课件，作为"评估集要评审"的实例）**：初版有一题「凌晨一点收工打车回家，这笔钱能报吗？」标 gold = C03（市内交通），**三路全"错"**——都指向 C12（加班夜间打车，语料里明写"加班到 21:00 之后打车可报销"）。**不是检索器坏了，是 gold 标错了**。教学点：**指标异常时先怀疑题。**
- **实证补记（用户完成挑战 A/B + 收束判卷，2026-09-12）**：
  - **挑战 A（给尺子加刻度）**：① 加 5 道自己出的题（22 → **27 题**，多 gold 题让 Recall 与 Hit 分家）；② 删掉 C12 再跑，观察哪些题 top-1 变了；③ 加一路"只看块标题"的检索器（路 E）。**踩到一个漂亮的坑**：路 E 初版写成 `1.0 if query in title else 0.0` = **整句子串判断**——整句话永远不是标题的子串，于是 29 块全拿 0.0，`rank_of` 的稳定排序**退化成文件顺序**（每题 top-3 恒为 C01 C02 C03）。**排序失效 ≠ 检索器差；看指标前先确认它真的在排序。**
  - **挑战 B ①（标定 `MIN_SCORE`）**：独立写出离线标定脚本 `code/_calibrate-min-score.py`（产物是一个数，运行时不跑它——真实 RAG 项目就是这么分工的），并**自己补出了第二类负样本**：0009 镜 2 只量了【噪音天花板】，量不到"**手册里根本没有**的问题该不该拒答"。30 块 / 27 题上的三条线：正样本最低 **0.424**、噪音最高 **0.699**、孤儿最高 **0.529** → **重叠**。阈值表：`0.70 → 误杀 21 (78%)`；**`0.60 → 误杀 8 (30%) / 漏放噪音 4 (15%) / 漏放孤儿 0`**；`0.50 → 漏放噪音 24 (89%)`。**定 0.60 的理由（这就是"标定"和"拍脑袋"的区别）**：误杀 = 诚实拒答、漏放 = 拿着噪音编，**两者代价不对称**，所以往"更怕漏放"这一侧偏。
  - **挑战 B ②（8 块 → 30 块搬进 0001）**：实测出两条**方向相反**的结论——① **余弦只取决于 query 和 chunk 两个向量，所以"加块不会改变已有块的分数"**（上一课那句原话 0.731、名次 1，一字节没变）；② **但"噪音天花板"是取 max，候选池变大 → max 只涨不跌**（实测 **22/27 道题**的噪音天花板在 8→30 块时上涨）→ **两条分布只会越来越重叠，阈值必须重标**。这是"阈值随语料扩张而失效"的机制级解释。
  - **一个值得单独记住的观察**：那 8 道被 0.6 误杀的题（**429 / RTO / 2FA / 比价 / 代码评审 / 覆盖率 / 学习基金 / 代码合入**）**全是术语型、精确串型**；逐题复核后确认**它们连噪音块也都在 0.60 以下**，所以不会"答错"，只会退化成一次**诚实的拒绝**。→ 这直接把"阈值定错了吗"翻译成了正确的问题：**这 8 题的根因不是阈值，是 0001 当时只有语义一路、没有合流**。
  - **挑战 B ③ + 检索质量改进（用户确认后由教师代笔落地并预跑取证，用户尚未亲跑 REPL；`code/0001-minimal-agent.py`）**：
    - **检索层升级为「关键词路 + 语义路 → RRF 合流」**。**这一步撞上的真问题**：阈值该挂在哪？RRF 只吃名次，合流后的结果是名次不是分数，**没法再拿 0.6 去卡它**。落地做法：**闸门留在单路，合流只管排名**——语义路低于 `MIN_SCORE`、关键词路低于 `MIN_SCORE_KW` 的块，**这一路不投它的名次**；**拒答判据变为"两路都没投出任何块"**。**两条量纲、两个数**（0.6 余弦 / 0.15 重合率），第三次实锤"量纲不可跨路搬运"。
    - **量出来的设计取舍（27 题 + 5 道孤儿题）**：

      | 方案 | Hit@1 | Hit@2 | top-2 混进非答案块 | 孤儿题正确拒答 |
      |---|---|---|---|---|
      | A · 闸门留在单路（**采用**） | 0.81 | 0.85 | **8/27** | 5/5 |
      | C · 闸门只当拒答判据、排名不设闸 | 0.81 | 0.85 | 18/27 | 5/5 |

      **Hit 完全一样，C 让噪音进 prompt 的题翻了一倍多** → 按文件自己写的"更怕漏放"取 A。合流把原先被误杀的 8 题救回 **3 道**（01 年假 / 04 2FA / 18 学习基金）。
    - **★ 挖到一个教学价值极高的失败案例（题 02「RTO 是多少」合流后给了 C06、gold C21 掉第 2）**：根因**不是合流**，是**关键词路的 2-gram 打分在短查询上打平、而并列由文件顺序断**——query 的 6 个 bigram = `{RT, TO, "O␣", "␣是", 是多, 多少}`；C21「…（RTO）为 2 小时」命中 `RT、TO` = 2/6 = 0.333；C06「…先经 CTO 书面批准」命中 `TO、O␣` = 2/6 = **0.333**（**"CTO"不是"RTO"，纯属巧合**）。`rank_of` 用稳定排序 → 并列按 `CHUNKS` 文件顺序断 → C06 定义得更早，排第 1。**一个真词和一个巧合被打成平手，RRF 再把这个"假的名次差"放大成胜负。** 更值得记的是：**语义路被 0.6 闸空后合流退化成关键词单路**，连"另一路的弱票"都没有了（0009 镜 1 里同一题混合路 ✅C21 排第 1，正因那边不设闸）。**补法按本课自己的规矩：这是检索质量问题，不是阈值问题**——该换更强的字面匹配（分词 / BM25 / IDF），而不是把 0.6 往下调。⚠️ **但这题实际答对了**——k=2 把 C21 也带进了材料，是 Hit@1 掉一名、效果没掉：**只看 Hit@1 会误判**。
    - **显式的一步查询改写**（`SYS_REWRITE` + `rewrite_query`，抄自 0009 镜 3），接在进 agentic loop 之前，**改写前后原样打印**。真机实测：`住院能报多少？` → 追问 `那门诊那部分呢？` → 改写成「员工补充商业医疗险门诊费用报销比例和每人每年报销上限是多少」→ 命中 C04 → **门诊 80% / 上限 5000 元 ✅**（这次没把"门诊"改偏）。`RTO 是多少` 答对 ✅；`食堂几点开饭？` **两路都没证据 → 诚实拒答 ✅**。**代价**：这步**每轮多调一次模型**（账单上看得见）；另踩一个小坑：**改写有时只差一个标点**，会让"其实没改"被判成改写过 → 比较前先去空白与句末标点。
  - **收束自释判卷（2026-09-12）**：**8 个空全中，1 处笔误**（"自己**表**的评估**价**" → 应是"自己**标**的评估**集**"）。另**三处可以更锋利**：① 负样本其实有**三类**不是两类（噪音天花板 / **孤儿问题**——后者是用户自己在 `_calibrate` 里补出来的，0009 镜 2 量不到）；② "分不开时该做提升检索质量"少了半句——**还可以按代价不对称明说选哪一边**，唯独不该继续试数字；③ "改写要用 eval 验收"要对准——**改写本身也会弄丢、会理解偏（命中 ≠ 改写正确）**。教师复核了用户写的每个实测数字（重跑 `_calibrate`）：三条线、两张阈值表、`22/27 噪音天花板上涨`、`8 题无噪音漏放` **全部对得上**。

### ⑤ 工程陷阱与面试追问
- 面试题「你怎么知道你的 RAG 变好了？」→ **第一句："得先有评估集"**——一份自己标的题集（题 + gold）+ Hit@1/Hit@k/Recall@k/MRR。没有它，你只能看到"变化"、看不到"改进"。**能主动说出"这把尺子自己也有误差（偏乐观 / 分辨率 / 过拟合）"比背指标名高一档。**
- 面试题「混合检索怎么做？为什么用 RRF？」→ RRF 只吃**名次**不吃分数，绕开不同检索路的**量纲**问题（关键词的重合率 vs 余弦，量级完全不同）；**主动补一句它的边界**：RRF 治漏不治排第一，收益在 Hit@k / Recall@k 而非 Hit@1。
- 陷阱① **把"评估"理解成"让模型自评"或"看着答案感觉还行"** → 评估 = **自己标的 gold + 客观指标**，机器判分、可复现；模型自评是另一回事且不稳。
- 陷阱② **以为指标下降 = 检索器变差** → 指标是**被题集定义**的：加了一批更难的题，指标必然掉，**那是尺子变细了**。
- 陷阱③ **抄别人项目的阈值** → 必须按自己数据的正/负样本分布标定；别人的数、别的语料、别的 embed 模型都不能搬；**甚至自己语料从 8 块扩到 30 块，那个数就失效**（本课实测）。
- 陷阱④ **以为"混合 = 全面更强"** → RRF 会把单路的错答案顶上来；且**某一路被闸空时合流会退化成单路**，单路自己的排序毛病就没人兜了。
- 陷阱⑤ **只看一个指标** → Hit 满分而 Recall 0.33 是最典型的自我感觉良好。
- 陷阱⑥ **赌模型自发改写追问** → 必须显式做，并用同一把尺子验收（改写也可能弄丢原本能命中的查询）。
- 陷阱⑦ **命中 = 改写正确** → 实测有一句改写把意思理解偏了，只因主题词还在就照样捞对；**改写质量仍要人看**。
- 陷阱⑧ **k 越大越好 / 越小越省** → 看曲线拐点（本课 k=3 之后指标全平、token 涨到 8 倍）。
- 陷阱⑨ **只看均值不看分歧** → 评估集里真正推动决策的是"各路给出不同答案"的那几道题；全票通过的题对选型没有信息量。

### ⑥ 术语·厂商对照·原典
- 术语：eval / evaluation set / gold（ground truth）/ Hit@k / Recall@k / MRR / nDCG / RRF（Reciprocal Rank Fusion）/ rerank / top_k / threshold（分数下限）/ query rewriting / query 自足性 / 误杀（false negative）/ 漏放（false positive）/ 回归测试集
- 厂商对照：**评估这一环是跨厂商的通用工程能力**。Hit@k / Recall@k / MRR / nDCG 是信息检索几十年的通用指标（学术基准 BEIR 也用它），**不随向量库换代**；RRF 是混合检索的默认融合法（Elasticsearch / Weaviate / Qdrant 等均有实现）。指标清单的"检索端 vs 生成端"分法与 Ragas 等框架一致（本课量检索端，生成端的 faithfulness / **Response Relevancy** 留 M6）。
- 原典（必读）：Anthropic **Demystifying evals for AI agents**（2026-01-09，目前最权威的 agent 评估一手文：eval = 输入 + 评分逻辑；难点在单轮 → 多轮 → agentic 的迁移，工具调用改状态会让误差累积放大）；Anthropic **Introducing Contextual Retrieval**（本课镜 1 题 02 是它"embedding 会漏精确串 → 需与 BM25 双轨合流"的现场证据；实测单加上下文前缀降 49%、再加 rerank 降 67%）；Anthropic **Effective context engineering for AI agents**（补最后一层："怎么知道你的上下文策略是对的"）；**Ragas** 官方文档（工业界 RAG 评估的指标清单）。延伸：Anthropic 官方文档 Test & evaluate 一节、OpenAI evals 指南、BEIR 检索基准。**网络提示**：`platform.claude.com`（Anthropic API 文档）本机被地区封锁、`developers.openai.com` 反爬 403——**浏览器能开，别因抓不到就改链接**。

---

*本文件已积累：第 1 课（M1）✅ ｜ 第 2 课（M2）✅ ｜ 第 3 课（M2）✅ ｜ 第 4 课（M2）✅ ｜ 第 5 课（M2）✅ ｜ 第 6 课（M2 收官）✅（2026-09-07 成本实证 + 用户挑战 A/B 完成回填）｜ 第 7 课（M3）✅（2026-09-08 实证回填：三镜 + 挑战 A/B + 收束自释判卷）｜ 第 8 课（M3）✅（2026-09-10 实证回填：三镜 + 挑战 A/B + 分数下限实验 + 收束判卷）｜ 第 9 课（M3 收官）✅（2026-09-12 实证回填：四镜 + 挑战 A/B + 合流与改写代笔预跑 + 并列名次失败案例 + 收束判卷）。**M3「记忆与上下文」三课收官（管得住 → 取得回 → 量得准）**，完成新课请按顶部模板追加一节。*
