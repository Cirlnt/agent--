...
print("""
  ...
  最后一行的"全量塞 30 块"是第 8 课那面镜子，放在一起才知道"省下来的到底是什么"。
""")

print(f"    {'k':>3} {'Hit@1':>8} {'Hit@k':>8} {'Recall@k':>9} {'MRR@5':>8} {'平均注入 token':>16}")
SCAN_KS = (1, 2, 3, 4, 5, 8)
TOK_KS = (1, 3, 5, 8, 30)                      # 量 token 的几档；30 = 全量塞（第 8 课的对照组）
...
for k in SCAN_KS:
    ...
    tail = "   ← 全量塞（第 8 课那面镜子）" if k == 30 else ("   ← 本课默认" if k == 3 else "")
    print(f"    {k:>3} {hit1:>8.2f} {hitk:>8.2f} {reck:>9.2f} {mrr:>8.2f} {tok}{tail}")
print(f"    全量塞 30 块：平均注入 {tok_map[30]:>3} token —— 是 k=3 的 {tok_map[30]/tok_map[3]:.1f} 倍，"
      f"而上面每一栏指标一个点都没涨。")
print(f"    （token 一栏 = 6 道题的平均值，含注入原文 + 问题；本地 embedding ¥0，只有这部分走付费模型。）")
print("""
  ★ 结论：k 的收益是【边际递减】的——Hit@k 到 k=2 就满了、Recall@k 到 k=3 也满了；
    再往后 5、8、30 一路加上去，指标纹丝不动，token 却从 197 涨到 1576（8 倍）。
...
print(f"""
  ...
  3. 阈值怎么定：画正/负样本两条分布 → 看重叠区 → 按场景取舍。抄别人项目的阈值 = 没做标定。
     （上一课那个 0.6 拿到 30 块语料上，正样本误杀率见镜 2 —— 语料一变，阈值就得重标。）
  ...
  诚实提醒（一定要会说的话）：这把尺子有误差，读数字前先看清三件事——
    ...
    ② 分辨率：{len(QUESTIONS)} 道题上，名次差一名 = {1/len(QUESTIONS):.1%}。所以上面 0.91 与 0.95 之间
       只差【一道题】——这个量级的差距【不足以】下结论；要大到超过一两道题的噪音才值得信。
